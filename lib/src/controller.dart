...
class AppleMapController {
  ...
  Future<ApplePoiDetail?> reverseGeocode(LatLng latLng) async {
    final detail = await channel.invokeMapMethod<String, dynamic>(
        'map#reverseGeocode', <String, dynamic>{
      'annotation': [latLng.latitude, latLng.longitude]
    });
  ...
  Future<List<ApplePoiDetail>?> searchRegion(
    LatLng latLng,
    String search,
    double radius,
  ) async {
    final list = await channel
        .invokeMapMethod<String, dynamic>('map#searchRegion', <String, dynamic>{
      'annotation': [latLng.latitude, latLng.longitude],
      'search': search,
      'radius': radius,
    });
