...
class FlutterAnnotation: NSObject, MKAnnotation {
    ...
    public init(fromDictionary annotationData: Dictionary<String, Any>, registrar: FlutterPluginRegistrar) {
        ...
        if let count = annotationData["count"] as? Int, count > 0 {
            self.count = count
        } else {
            self.count = nil
        }
